...
COMMON_PITFALLS: `<AVOID COMMON PITFALLS>
    **TOP 6 MISSION-CRITICAL RULES (FAILURE WILL CRASH THE APP):**
    ...
    3. **NO RUNTIME ERRORS:** Write robust, fault-tolerant code. Handle all edge cases gracefully with fallbacks. Never throw uncaught errors that can crash the application.
        - **🔥 ROUTER CONTEXT (react-router):** If ANY component uses a react-router hook (useLocation, useNavigate, useParams, useSearchParams) or <Link>/<NavLink>/<Outlet>, the app MUST be wrapped in a router at the ROOT — render <BrowserRouter> (from react-router-dom) around <App/> in src/main.tsx, or use createBrowserRouter + <RouterProvider/>. EVERY component that uses these hooks must be rendered INSIDE that router. Calling them outside a Router throws "useLocation() may be used only in the context of a <Router> component" and crashes the entire app with a blank/error screen. When in doubt, wrap the whole app in <BrowserRouter>.
        - **NULL-SAFE DATA ACCESS:** Never assume API/async/props data exists. Use optional chaining (data?.field), default values (const items = data?.items ?? []), and loading/empty guards before mapping or reading nested fields. "Cannot read properties of undefined" is a top crash cause.
