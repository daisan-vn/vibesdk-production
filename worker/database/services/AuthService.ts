...
export class AuthService extends BaseService {
     ...
     */
    async ensureDemoUser(): Promise<AuthUser> {
        const existing = await this.getUserForAuth(DEMO_USER_ID);
        if (existing) {
            return existing;
        }
        ...
        try {
            ...
            logger.info('Created demo user', { userId: DEMO_USER_ID });
        } catch (error) {
            logger.warn('Demo user insert skipped (may already exist)', { error });
        }

        const user = await this.getUserForAuth(DEMO_USER_ID);
        if (!user) {
            throw new SecurityError(
                SecurityErrorType.INVALID_INPUT,
                'Failed to ensure demo user',
                500,
            );
        }
        return user;
