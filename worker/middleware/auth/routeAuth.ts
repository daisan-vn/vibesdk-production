...
import { getDemoUserSession, isDemoModeEnabled } from '../../services/demo/demoAuth';
import { isDemoAccessToken } from 'shared/constants/demo-auth';
...
export async function enforceAuthRequirement(c: Context<AppEnv>) : Promise<Response | undefined> {
    ...
    if (!user && (requirement.level === 'authenticated' || requirement.level === 'owner-only')) {
        ...
        // Strategy 2: Standard JWT auth (header/cookie) or demo token
        if (!user) {
            let userSession = await authMiddleware(c.req.raw, c.env);

            if (!userSession && isDemoModeEnabled(c.env)) {
                const token = extractToken(c.req.raw);
                if (!token || isDemoAccessToken(token)) {
                    userSession = await getDemoUserSession(c.env);
                }
            }
